...
`go-ruby-typhoeus/typhoeus` reimplements Ruby's `typhoeus` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-typhoeus/typhoeus` directly, with no Ruby runtime;
...
Because `typhoeus` is pure Go with cgo disabled, it also compiles to **WebAssembly**
